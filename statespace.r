...

{
  ...
  codes <- readxl::read_xlsx("../data/codes.xlsx") # here I bring the codes
  #
  regions <- codes %>% filter(Variable == "region") %>% pull(Code)
  regions.nm <- codes %>% filter(Variable == "region") %>% pull(Name)
  names(regions) <- regions.nm
  nreg <- length(regions)
  
  origins <- codes %>% filter(Variable == "origin") %>% pull(Code)
  origins.nm <- codes %>% filter(Variable == "origin") %>% pull(Name)
  names(origins) <- origins.nm
  nori <- length(origins)
  
  educodes<- codes %>% filter(Variable == "edu") %>% pull(Code)  
  edunames <- codes %>% filter(Variable == "edu") %>% pull(Name)
  names(educodes) <- edunames
  nedu <- length(educodes)
  
  ages <- codes %>% filter(Variable == "age") %>% pull(Code) %>% as.numeric()
  agesnames <- codes %>% filter(Variable == "age") %>% pull(Name)
  names(ages) <- agesnames  
  nage <- length(ages)
  
  sexcodes <- codes %>% filter(Variable == "sex") %>% pull(Code)  
  sexnames <- codes %>% filter(Variable == "sex") %>% pull(Name)
  names(sexcodes) <- sexnames 
  nsex <- length(sexcodes)
  
  saveRDS(regions, "../data/regions.rds")
  saveRDS(origins, "../data/origins.rds")
  saveRDS(educodes, "../data/edu.rds")
  saveRDS(ages, "../data/ages.rds")
  saveRDS(sexcodes, "../data/sex.rds")
  ...
  fertages <- codes %>% filter(Variable == "age") %>% pull(Code) %>% .[. >= 15 & . <= 45 & !. %in% c(5, 100)] %>% as.numeric()
...
#  nedu = length(educodes)
  eduages <- codes %>% filter(Variable == "age") %>% pull(Code)  %>% .[. >= 15 & . <= 25& !. %in% 100] %>% as.numeric()
 #eduages = seq(15,25,by=5) #transitions from these ages
...
if(iscen!="baseline"){ #ignore
  ...
  for(i in c("sx","imr","emr","dimr","demr")){ #sx imr emr
    #sx/imr/emr
  ...
  for(i in "asfr"){ #asfrdt
    xxxdt <- expand.grid(region = regions,
                         origin = origins,
                         Time=pers[-length(pers)],
                         sex=sexcodes[2],edu=educodes,agest=fertages,stringsAsFactors=F)
    setDT(xxxdt)
    xxdtname <- paste(i,"dt",sep="")
    assign(xxdtname,xxxdt);rm(xxxdt)
  }
  ...
  for(i in "prop"){#eaprlmdt
    ...
    xxxdt <- expand.grid(region = regions,
                         ...
                         Time=pers, 
                         sex=sexcodes,
                         edu=educodes,agest=eduages, #for proportion
                         stringsAsFactors=F)
    setDT(xxxdt)
    xxdtname <- paste(i,"dt",sep="")
    assign(xxdtname,xxxdt);rm(xxxdt)
  }
  # edu=educodes,agest=eduages+5, #for proportion  --> why the "+5"
